...
double calculateAverage(unsigned long int array[], unsigned int long size);
void readData(const char* filename, unsigned long int array[], unsigned long int size); 
...
double calculateAverage(unsigned long int array[], unsigned long int size) {
...
inline void print_results(Point* list, int num_of_pts, char* str) {
	char name[15] = "points";
	strcat(name, str);
	strcat(name, ".txt");
	FILE* fd = fopen(name, "w");
