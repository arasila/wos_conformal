...
inline double walk_on_spheres(double x, double y, unsigned int num_shots, double epsilon){
	...
	while (j < num_shots) {
		...
		if (x < 0) x = -x;  // Out of Neumann boundary x=0, 0<=y<=2, Reflection
		else if (x > 6) x = x - 6;  // Out of Neumann boundary x=6, 0<=y<=2, Reflection and translation
		else if (2 < x && x < 4 && y > 1) y = 2 - y;  // Out of Neumann boundary y=1, 2<=x<=4, Reflection
		...
		if (y <= 1) {
			...
			r = (dist1 < dist2) ? ((dist1 < dist3) ? dist1 : dist3) : ((dist2 < dist3) ? dist2 : dist3);
		} else {
			if (x < 2) {
			    r = (2 - y < 2 - x) ? (2 - y) : (2 - x);
			} else if (x > 4) {
