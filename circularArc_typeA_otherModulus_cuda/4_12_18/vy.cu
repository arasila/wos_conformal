...
#define center2x -0.41421356237309515
#define center2y 1.0
#define r2 0.41421356237309515
#define center4x 1.0
#define center4y 0.2679491924311228
#define r4 0.2679491924311228
#define Tarc2Centerx 0.9570549972639842
#define Tarc2Centery 0.29017916163123975
#define Tarc2Radius 0.012578300083302517
#define Tarc4Centerx -0.3157433184397003
#define Tarc4Centery 0.9490280509882943
#define Tarc4Radius 0.01865702821868881
...
int main(int argc, char** argv) {
    ...
    CHECK_CUDA(cudaMalloc((void**)&d_fm2h, NUM_POINTS * sizeof(unsigned int)));
    CHECK_CUDA(cudaMalloc((void**)&d_fmh,  NUM_POINTS * sizeof(unsigned int)));
    CHECK_CUDA(cudaMalloc((void**)&d_fph,  NUM_POINTS * sizeof(unsigned int)));
    CHECK_CUDA(cudaMalloc((void**)&d_fp2h, NUM_POINTS * sizeof(unsigned int)));

    const int TPB    = 256;
    const int blocks = NUM_POINTS;
    unsigned long long seed = (unsigned long long)time(NULL);

    vy_kernel<<<blocks, TPB, 4 * TPB * sizeof(unsigned int)>>>(NUM_POINTS,
                               x1, y1,
                               x2, y2,
                               x3, y3,
                               x4, y4,
    ...
    CHECK_CUDA(cudaMemcpy(fm2h_list, d_fm2h, NUM_POINTS * sizeof(unsigned int), cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(fmh_list,  d_fmh,  NUM_POINTS * sizeof(unsigned int), cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(fph_list,  d_fph,  NUM_POINTS * sizeof(unsigned int), cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(fp2h_list, d_fp2h, NUM_POINTS * sizeof(unsigned int), cudaMemcpyDeviceToHost));
