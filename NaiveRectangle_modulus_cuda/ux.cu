// ux.cu
// Recommended build (RTX 4090): nvcc -O3 -arch=sm_89 --use_fast_math ux.cu -o ux -lcurand
// Run:   ./ux
...
__device__ unsigned int walk_on_spheres_device(double x1, double y1,
    ...
    while (j < num_shots) {
        if (wy < 0.0) {
            wy = -wy;
        } else if (wy > 2.0 * HEIGHT) {
            wy = 2.0 * HEIGHT - wy;
        }
        double r = (1.0 - wx) > wx ? wx : (1.0 - wx);
        if (r < EPSILON) {
            if (wx > 0.5) mark++;
            wx = x1;
            wy = y1;
            j++;
            continue;  // start a new shot, do not perform another step with this tiny radius
        }
...
__device__ unsigned int walk_on_spheres_strided(double x1, double y1,
    ...
    while (j < NN) {
        ...
        if (r < EPSILON) {
            ...
            wy = y1;
            j += j_stride;
            continue;  // start next strided shot, skip the extra step
        }
        double u1 = curand_uniform_double(state) * 2.0 * PI;
        double z1 = sin(u1);
        double z2 = cos(u1);
...
/* kernel: one block per point, threads share shots via striding and shared-memory reduction */
__global__ void wos_kernel(int n,
    ...
    int p = blockIdx.x;  // one block per sampling point index
    if (p >= n) return;
...
int main() {
    ...
    cudaMemset(d_fm2h, 0, N * sizeof(unsigned int));
    cudaMemset(d_fmh,  0, N * sizeof(unsigned int));
    cudaMemset(d_fph,  0, N * sizeof(unsigned int));
    cudaMemset(d_fp2h, 0, N * sizeof(unsigned int));

    int threads = 256;
    int blocks  = N;  // one block per point
    unsigned long long seed = (unsigned long long)time(NULL);
    size_t shmem_size = 4 * threads * sizeof(unsigned int);

    wos_kernel<<<blocks, threads, shmem_size>>>(
        N,
        x1, y1,
        x2, y2,
        x3, y3,
