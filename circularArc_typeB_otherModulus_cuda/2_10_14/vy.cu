...
unsigned int walk_on_spheres(double x, double y, unsigned int num_shots, double epsilon); 
...
void print_fm2h_results(unsigned int* list, int num_of_pts);
void print_fmh_results(unsigned int* list, int num_of_pts);
void print_fph_results(unsigned int* list, int num_of_pts);
void print_fp2h_results(unsigned int* list, int num_of_pts);
                                            ...
                                            curandStatePhilox4_32_10_t* rng)
{
    ...
    while (j < (int)NN) {
        ...
        if (wy >= 0 &&
            !((tan(2.0 / 24.0 * PI)*wx < wy  && wy < wx*tan(10.0 / 24.0 * PI)) ||
              wx*tan(7.0 / 12.0 * PI) > wy)) {
            distance = sqrt(fmin(
                (wx - 1.0) * (wx - 1.0) + wy * wy,
                fmin(
...
__device__ unsigned int walk_on_spheres_dev_strided(double x1, double y1,
                                                    unsigned int NN, double eps,
                                                    curandStatePhilox4_32_10_t* rng,
                                                    ...
                                                    unsigned int j_stride)
{
    ...
    for (unsigned int j = j_start; j < NN; j += j_stride) {
        double wx = x1, wy = y1;
        double distance, theta, denom;
                          ...
                          unsigned long long seed)
{
    ...
    curandStatePhilox4_32_10_t rng;
    curand_init(seed, (unsigned long long)global_i * blockDim.x + tid, 0ULL, &rng);


    unsigned int local_m2 = walk_on_spheres_dev_strided(x1, y1, num_shots, eps, &rng,
                                                        (unsigned int)tid, (unsigned int)blockDim.x);
    unsigned int local_m1 = walk_on_spheres_dev_strided(x2, y2, num_shots, eps, &rng,
                                                        (unsigned int)tid, (unsigned int)blockDim.x);
    unsigned int local_p1 = walk_on_spheres_dev_strided(x3, y3, num_shots, eps, &rng,
                                                        (unsigned int)tid, (unsigned int)blockDim.x);
    unsigned int local_p2 = walk_on_spheres_dev_strided(x4, y4, num_shots, eps, &rng,
                                                        (unsigned int)tid, (unsigned int)blockDim.x);
...
unsigned int walk_on_spheres(double x1, double y1, unsigned int NN, double eps) {
    double wx = x1, wy = y1;
    unsigned int cnt = 0;
    int j = 0;
    double distance, theta, denom;
