...
#define NUM_SHOTS 1.0E5 /* Must be a float number */
#define PI 3.14159265358979323846
#define EPSILON 1e-6
#define length(x, y) sqrt((x)*(x)+(y)*(y))

#define v1_x 0.7071067811865475
#define v1_y 0.7071067811865475
#define v2_x -0.5
#define v2_y 0.8660254037844386
#define v3_x -1.0
#define v3_y 0.0
#define v4_x 1.0
#define v4_y 0.0
...
inline double walk_on_spheres(double x1, double y1, unsigned int NN, double eps) {
	...
	while (j < NN) {
		...
		if((wx >= 0 && (0 <= wy && wy <= wx)) || (wx < 0 && (0 < wy && wy < wx * tan(2 * PI / 3)))) {
...
inline Point* get_points(int* num_of_pts) {
    ...
    //for(int i=0; i<counter; i++) printf("counter = %d, list[%d]->x = %lf, list[%d]->y = %lf, length = %lf\n",counter,i,list[i]->x,i,list[i]->y, length(list[i]->x,list[i]->y));
