// wos_inversion_cuda.cu
// CUDA port of source.c (Walk-on-Spheres with circle inversions & arcs)
// Build: nvcc -O3 -arch=sm_89 source.cu -o source -lcurand
// Run:   ./source
...
__device__ double WalkOnSpheres_dev(double x1,
                                    double y1,
                                    unsigned int NN,
                                    double eps,
                           ...
                           unsigned long long seed)
{
    ...
    double val = WalkOnSpheres_dev(x, y, num_shots, eps, &rng);
    results[tid] = val;
}
...
inline Point* get_points(int* num_of_pts) {
    ...
    // Smoothen the boundary: refined sampling on two arcs
    for (int i = 0; i <= 500; i++) {
